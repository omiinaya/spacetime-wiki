...
const sampleUsers = [
  { id: "u1", name: "Alice", email: "alice@example.com", role: "member", avatar_url: "", created_at: 0, updated_at: 0, password_hash: "" },
  { id: "u2", name: "Bob", email: "bob@example.com", role: "member", avatar_url: "", created_at: 0, updated_at: 0, password_hash: "" },
];
...
describe("SearchFilters", () => {
  ...
  it("calls onChange when date filters change", async () => {
    ...
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ dateTo: "2026-06-28" }),
    );
  });
  ...
  it("calls onChange when tags input changes", async () => {
    ...
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ tags: "draft, meeting" }),
    );
  });
