...
describe("AdminDashboard", () => {
  ...
  it("shows 'No pages created yet' when contributors empty", async () => {
    mockSqlQuery
      .mockResolvedValueOnce([{ c: 0 }]).mockResolvedValueOnce([{ c: 1 }])
      .mockResolvedValueOnce([{ c: 0 }]).mockResolvedValueOnce([{ c: 0 }])
      .mockResolvedValueOnce([{ c: 0 }]).mockResolvedValueOnce([{ c: 0 }])
      .mockResolvedValueOnce([{ c: 0 }]).mockResolvedValueOnce([{ c: 0 }])
      .mockResolvedValueOnce([{ c: 0 }]).mockResolvedValueOnce([{ total: 0 }])
      .mockResolvedValueOnce([]);
    render(<AdminDashboard userId="admin1" />);
    await waitFor(() => {
  ...
  it("renders recent activity items", async () => {
    mockSqlQuery
      .mockResolvedValueOnce([{ c: 5 }]).mockResolvedValueOnce([{ c: 1 }])
      .mockResolvedValueOnce([{ c: 2 }]).mockResolvedValueOnce([{ c: 3 }])
      .mockResolvedValueOnce([{ c: 0 }]).mockResolvedValueOnce([{ c: 4 }])
      .mockResolvedValueOnce([{ c: 1 }]).mockResolvedValueOnce([{ c: 0 }])
      .mockResolvedValueOnce([{ c: 0 }]).mockResolvedValueOnce([{ total: 0 }])
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([
        { id: "e1", event_type: "page.create", actor_id: "u1", target_name: "My Page", created_at: 1700000000 },
        { id: "e2", event_type: "comment.create", actor_id: "u2", target_name: "Other Page", created_at: 1699900000 },
      ]);
    render(<AdminDashboard userId="admin1" />);
    await waitFor(() => {
