...
describe("htmlToProseMirror", () => {
  ...
  it("converts image inside figure", () => {
    const result = htmlToProseMirror('<figure><img src="https://example.com/img.png" alt="pic" /></figure>');
    expect(result.content[0]).toMatchObject({
      type: "image",
  ...
  it("converts various inline formatting (bold, italic, code, strike, link)", () => {
    const html = '<p><strong>Bold</strong> <em>Italic</em> <code>Code</code> <s>Strike</s> <a href="https://x.com">Link</a></p>';
