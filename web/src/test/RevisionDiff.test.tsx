...
describe("RevisionDiff", () => {
  ...
  it("shows addition and removal counts", () => {
    renderDiff(baseRevision, newRevision);
    expect(screen.getByText((content) => content.includes("+1") && content.includes("addition"))).toBeInTheDocument();
    expect(screen.getByText((content) => content.includes("0") && content.includes("removal"))).toBeInTheDocument();
  });
  ...
  it("shows 0 additions and 0 removals when content unchanged", () => {
    renderDiff(noDiffRevisions[0], noDiffRevisions[1]);
    expect(screen.getByText((content) => content.includes("+0") && content.includes("addition"))).toBeInTheDocument();
    expect(screen.getByText((content) => content.includes("0") && content.includes("removal"))).toBeInTheDocument();
  });
