...
describe("OidcCallback", () => {
  beforeEach(() => {
    ...
    removeItemSpy = vi.spyOn(Storage.prototype, "removeItem").mockImplementation((key: string) => { delete store[key]; });
  });
  ...
  it("shows error when URL has error param", async () => {
    ...
    render(<BrowserRouter><OidcCallback /></BrowserRouter>);
    await waitFor(() => {
  ...
  it("shows exchanging code status", async () => {
    ...
    __mockApi.oidc.get.mockResolvedValue({
      id: "provider-1", client_id: "client-abc", issuer_url: "https://idp.example.com", scopes: "openid email",
    });
  ...
  it("shows token error on OIDC token exchange failure", async () => {
    ...
    globalThis.fetch = vi.fn().mockImplementation(() => {
      callCount++;
      if (callCount === 1) {
        return Promise.resolve({ json: () => Promise.resolve({ token_endpoint: "https://idp.example.com/token", userinfo_endpoint: "https://idp.example.com/userinfo" }) });
      }
      return Promise.resolve({ json: () => Promise.resolve({ error: "invalid_grant", error_description: "Code expired" }) });
    });
  ...
  it("extracts email from id_token and signs in existing user", async () => {
    ...
    const payload = btoa(JSON.stringify({ email: "user@idp.com", name: "OIDC User", sub: "abc123" }));
    ...
    globalThis.fetch = vi.fn().mockImplementation(() => {
      ...
      return Promise.resolve({ json: () => Promise.resolve({ access_token: "at-123", id_token: idToken, error: undefined }) });
    });
