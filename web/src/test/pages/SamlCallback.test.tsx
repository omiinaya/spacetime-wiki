...
describe("SamlCallback", () => {
  beforeEach(() => {
    ...
    setItemSpy = vi.spyOn(Storage.prototype, "setItem").mockImplementation((key: string, value: string) => { store[key] = value; });
  ...
  it("shows error when no SAMLResponse param", async () => {
    ...
    render(<BrowserRouter><SamlCallback /></BrowserRouter>);
    await waitFor(() => {
...
<samlp:Response xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol"
  ...
  <saml:Assertion>
    ...
    __mockApi.saml.list.mockResolvedValue([{ id: "saml-1", entity_id: "https://saml.idp.example.com" }]);
    __mockApi.saml.get.mockResolvedValue({
      id: "saml-1", auto_register: true, attribute_mapping: '{"email": "mail", "name": "name"}',
    });
  ...
  it("signs in existing user via NameID", async () => {
    ...
    __mockApi.saml.list.mockResolvedValue([{ id: "saml-1", entity_id: "https://saml.idp.example.com" }]);
    __mockApi.saml.get.mockResolvedValue({
      id: "saml-1", auto_register: true, attribute_mapping: '{"email": "mail", "name": "displayName"}',
    });
  ...
  it("registers new user when auto_register is true", async () => {
    ...
    __mockApi.saml.list.mockResolvedValue([{ id: "saml-1", entity_id: "https://saml.idp.example.com" }]);
    __mockApi.saml.get.mockResolvedValue({
      id: "saml-1", auto_register: true, attribute_mapping: '{"email": "email", "name": "name"}',
    });
  ...
  it("shows error when auto_register is false and user doesn't exist", async () => {
    ...
    __mockApi.saml.list.mockResolvedValue([{ id: "saml-1", entity_id: "https://saml.idp.example.com" }]);
    __mockApi.saml.get.mockResolvedValue({
      id: "saml-1", auto_register: false, attribute_mapping: '{"email": "email", "name": "name"}',
    });
    __mockApi.users.getByEmail.mockResolvedValue(null);
  ...
  it("uses mapped attribute names from provider config", async () => {
    ...
    const resp = makeSamlResponse({ "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress": "mapped@idp.com" });
    ...
    __mockApi.saml.list.mockResolvedValue([{ id: "saml-1", entity_id: "https://saml.idp.example.com" }]);
    __mockApi.saml.get.mockResolvedValue({
      id: "saml-1", auto_register: true,
      attribute_mapping: JSON.stringify({ email: "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress", name: "name" }),
    });
