...
describe('LoginView', () => {
  ...
  it('renders LDAP provider section when available', async () => {
    __mockApi.ldap.listActive.mockResolvedValue([{ id: 'ldap-1', name: 'Corporate AD' }]);
  ...
  it('registers and then logs in successfully', async () => {
    ...
    await waitFor(() => {
      expect(__mockApi.users.register).toHaveBeenCalledWith(
        'Test User',
        'newuser@example.com',
        'newpassword',
        'member',
  ...
  it('switches to MFA backup code mode and verifies', async () => {
    ...
    const backupInput: HTMLInputElement = container.querySelector(
  ...
  it('triggers OIDC sign-in flow on click', async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({
      json: () =>
        Promise.resolve({ authorization_endpoint: 'https://github.com/login/oauth/authorize' }),
    });
    ...
    __mockApi.oidc.listActive.mockResolvedValue([
      {
        id: 'oidc-bad',
        name: 'BadIdP',
        issuer_url: 'https://badidp.example.com',
        client_id: 'xyz',
        scopes: 'openid',
      },
    ]);
