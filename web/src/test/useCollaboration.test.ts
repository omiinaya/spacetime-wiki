...
describe('useCollaboration', () => {
  ...
  it('returns isActive=false when pageId is undefined', () => {
    const { result } = renderHook(() => useCollaboration(undefined, 'user1', 'Alice'));
    expect(result.current.isActive).toBe(false);
  });

  it('returns isActive=false when userId is undefined', () => {
    const { result } = renderHook(() => useCollaboration('page1', undefined, 'Alice'));
    expect(result.current.isActive).toBe(false);
  });

  it('returns isActive=false when userName is undefined', () => {
    const { result } = renderHook(() => useCollaboration('page1', 'user1', undefined));
    expect(result.current.isActive).toBe(false);
  });

  it('creates a YjsStdbProvider when all params provided', async () => {
    const { result } = renderHook(() => useCollaboration('page1', 'user1', 'Alice'));
  ...
  it('applies remote updates when subscription data arrives', async () => {
    mockUpdatesData.mockReturnValue([{ update_data: 'AAAA' }]);
  ...
  it('destroys provider on unmount', async () => {
    const { result, unmount } = renderHook(() => useCollaboration('page1', 'user1', 'Alice'));
  ...
  it('toggles isActive based on session subscription changes', async () => {
    const { result, rerender } = renderHook(
      ({ pageId }: { pageId: string | undefined }) => useCollaboration(pageId, 'user1', 'Alice'),
