...
describe('Mermaid Node', () => {
  ...
  it('renders HTML with mermaid-wrapper class', () => {
    const html = getRenderHTML(node, {
      node: { attrs: { src: 'graph TD\n  A-->B' } },
      HTMLAttributes: {},
    });
...
describe('Transclusion Node', () => {
  ...
  it('renders HTML with transclusion attributes', () => {
    const html = getRenderHTML(node, {
      node: { attrs: { pageId: 'p1', pageTitle: 'My Page', content: null } },
      HTMLAttributes: {},
    });
    const [tag, attrs] = html;
    expect(tag).toBe('div');
...
describe('SyncedBlock Node', () => {
  ...
  it('renders HTML with synced-block attributes', () => {
    const html = getRenderHTML(node, {
      node: { attrs: { blockId: 'b1', blockTitle: 'Notes', content: null } },
      HTMLAttributes: {},
    });
    const [tag, attrs] = html;
    expect(tag).toBe('div');
...
describe('ImageEnhanced Node', () => {
  ...
  it('includes data-image-enhanced attribute on wrapper', () => {
    const html = getRenderHTML(node, {
      node: {
        attrs: { src: 'x.png', alt: null, title: null, width: null, align: 'center', caption: '' },
      },
      HTMLAttributes: {},
    });
    const [, attrs] = html;
  ...
  it('sets width on the inner img element', () => {
    const html = getRenderHTML(node, {
      node: {
        attrs: { src: 'x.png', alt: null, title: null, width: '50%', align: 'center', caption: '' },
      },
      HTMLAttributes: {},
    });
