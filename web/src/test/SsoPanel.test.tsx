...
const sampleOidcProviders = [
  {
    id: "oidc_1", name: "Keycloak", slug: "keycloak",
    issuer_url: "https://keycloak.example.com/auth/realms/myrealm",
    client_id: "spacetime-wiki", client_secret: "sec1", scopes: "openid email profile",
    is_active: true, created_by: "u1", created_at: 1000, updated_at: 1000,
  },
  {
    id: "oidc_2", name: "Okta", slug: "okta",
    issuer_url: "https://dev-123.okta.com",
    client_id: "okta_cid", client_secret: "sec2", scopes: "openid email",
    is_active: false, created_by: "u1", created_at: 900, updated_at: 900,
  },
];

const sampleSamlProviders = [
  {
    id: "saml_1", name: "Okta SAML", slug: "okta-saml",
    entity_id: "https://www.okta.com/saml2/service-provider/sp-wiki",
    sso_url: "https://okta.example.com/saml2/sso",
    certificate: "MIID...", name_id_format: "urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress",
    attribute_mapping: '{"email":"email","name":"name"}', auto_register: true,
    is_active: true, created_by: "u1", created_at: 1000, updated_at: 1000,
  },
];
...
describe("SsoPanel", () => {
  ...
  it("calls api.oidc.create with form data", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Provider name"), { target: { value: "My OIDC" } });
    fireEvent.change(screen.getByPlaceholderText("Slug (e.g. keycloak)"), { target: { value: "my-oidc" } });
    fireEvent.change(screen.getByPlaceholderText("Issuer URL"), { target: { value: "https://issuer.example.com" } });
    fireEvent.change(screen.getByPlaceholderText("Client ID"), { target: { value: "my-client-id" } });
    fireEvent.change(screen.getByPlaceholderText("Client Secret"), { target: { value: "my-secret" } });
  ...
  it("calls api.saml.create with form data", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Provider name"), { target: { value: "My SAML" } });
    fireEvent.change(screen.getByPlaceholderText("Slug"), { target: { value: "my-saml" } });
    fireEvent.change(screen.getByPlaceholderText("Entity ID / Issuer"), { target: { value: "https://saml.example.com/entity" } });
    fireEvent.change(screen.getByPlaceholderText("SSO URL"), { target: { value: "https://saml.example.com/sso" } });
  ...
  it("shows toast on OIDC create error", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Slug (e.g. keycloak)"), { target: { value: "test" } });
    fireEvent.change(screen.getByPlaceholderText("Issuer URL"), { target: { value: "https://issuer.example.com" } });
  ...
  it("has no accessibility violations in empty state", async () => {
    ...
    mockSamlList.mockResolvedValue([]);
    const { container } = renderSsoPanel();
    await waitFor(() => expect(screen.getByText("No OIDC providers configured.")).toBeInTheDocument());
