...
const sampleWebhooks = [
  ...
  {
    id: "wh2", name: "Discord Bot", url: "https://discord.com/api/webhooks/yyy",
    events: JSON.stringify(["page.delete"]), secret: "",
    is_active: false, created_at: 900, created_by: "u1",
  },
];

const sampleEvents = [
  { id: "e1", webhook_id: "wh1", event_type: "page.create", payload: "{}", status: "sent", response_code: 200, response_body: "{}", created_at: 1000 },
  { id: "e2", webhook_id: "wh1", event_type: "page.update", payload: "{}", status: "failed", response_code: 500, response_body: '{"error":"timeout"}', created_at: 900 },
];
...
describe("WebhookSettings", () => {
  ...
  it("calls api.webhooks.create when form submitted", async () => {
    ...
    await waitFor(() => {
      expect(mockCreate).toHaveBeenCalledWith("Test Hook", "https://example.com/hook",
        JSON.stringify(["page.create", "page.update"]), "", "u1");
    });
  });
  ...
  it("toggles webhook active status", async () => {
    ...
    await waitFor(() => {
      expect(mockUpdate).toHaveBeenCalledWith("wh1", "Slack Notifier",
