...
export function useBatchSelect(onRefresh: () => void) {
  ...
  const handlePageClick = useCallback(
    (pageId: string, e: React.MouseEvent, navigate: (path: string) => void) => {
  ...
  const handleBatchArchive = useCallback(async () => {
    ...
    addToast({
      type: 'success',
      title: `Archived ${selectedPageIds.size} page(s)`,
      duration: 3000,
    });
  ...
  const handleBatchDelete = useCallback(async () => {
    ...
    addToast({
      type: 'success',
      title: `Moved ${selectedPageIds.size} page(s) to trash`,
      duration: 3000,
    });
  }, [selectedPageIds, clearSelection, onRefresh, addToast]);
