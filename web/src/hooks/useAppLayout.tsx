...
import {
  FileText as FileTextIcon, BookOpen as BookOpenIcon, Plus as PlusIcon,
  FolderPlus as FolderPlusIcon, LayoutTemplate as LayoutTemplateIcon,
  Shield as ShieldIcon, Trash2 as Trash2Icon, Star as StarIcon,
  History as HistoryIcon, Share2 as Share2Icon, Code as CodeIcon,
  Keyboard as KeyboardIcon, Sun as SunIcon, Moon as MoonIcon,
} from "lucide-react";
...
export function useAppLayout() {
  ...
  const [contextMenu, setContextMenu] = useState<{
    x: number; y: number; colId?: string; pageId?: string
  } | null>(null);
  ...
  const [shareLinks, setShareLinks] = useState<
    { id: string; token: string; expires_at: number; visit_count: number;
      password_hash: string; brand_title: string | null; brand_logo_url: string | null }[]
  >([]);
  ...
  // ─── Effects ─────────────────────────────────────────────────────────────
  useEffect(() => { initGlobalToast(addToast); }, [addToast]);
  ...
  useEffect(() => { setPages(subPages); }, [subPages]);
  useEffect(() => { setCollections(subCollections); }, [subCollections]);
  ...
  useEffect(() => {
    ...
    if (subNotifications.length > prevNotifLenRef.current && prevNotifLenRef.current > 0) {
      ...
      if (latest && !latest.is_read) {
        showToast({
          type: "info", title: latest.title, message: latest.message, duration: 5000,
  ...
  const refreshNotifications = useCallback(() => {
    if (userId) { api.notifications.list(userId, 100).then(setNotificationList).catch(() => {}); }
  }, [userId]);
    ...
    if (prev.length > 0 && subPages.length > 0) {
      ...
      for (const page of subPages) {
        ...
        if (!was && page.created_by !== myId) {
          showToast({ type: "info", title: "New page created", message: `"${page.title}" was added`, duration: 5000,
          ...
          showToast({ type: "info", title: "Page updated", message: `"${page.title}" was modified`, duration: 4000,
            action: { label: "Open", onClick: () => window.location.assign(`/page/${page.id}`) },
          });
        }
      }
    ...
    const timer = setTimeout(async () => {
      if (loading) {
        try {
          ...
          setCollections(cols); setPages(allPages);
  ...
  const pagesByCollection: Record<string, Page[]> = (() => {
    ...
    for (const page of pages.filter((p) => p.status !== "deleted" && search.filteredPages.includes(p))) {
  ...
  const openCreateCol = () => {
    setEditingCol(null);
    setColName(""); setColDesc(""); setColIcon("📁"); setColColor("");
  ...
  const openEditCol = async (col: Collection) => {
    ...
    try {
      ...
      if (rule) {
        const mode = rule.sort_field === "manual" ? "manual" : rule.sort_field + "-" + rule.sort_direction;
        setColSortMode(mode);
  ...
  const loadTrashPage = async () => {
    ...
    try { setTrashPages(await api.pages.listDeleted()); }
  ...
  const openShareDialog = async (pageId: string, pageTitle: string) => {
    try { setShareLinks(await api.shareLinks.list(pageId)); } catch {}
  ...
  const openTemplates = async () => {
    try { setTemplates(await api.pages.listTemplates()); }
    catch (e) { console.error(e); }
  ...
  const handleExportPageHTML = async (pageId: string) => {
    try {
      ...
      const json = JSON.parse(page.content || "{}");
      const md = tiptapToMarkdown(json);
      const styledMd = md.split("\n").map(l => {
        if (l.startsWith("#")) return `<h${l.match(/^#+/)?.[0]?.length || 1}>${l.replace(/^#+\s*/, "")}</h${l.match(/^#+/)?.[0]?.length || 1}>`;
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      try {
        const hasFilters = search.searchFilters.collectionId || search.searchFilters.authorId ||
          search.searchFilters.dateFrom || search.searchFilters.dateTo || search.searchFilters.tags;
        if (search.searchQuery || hasFilters) {
          const result = await api.pages.search({
            q: search.searchQuery || "",
            ...(search.searchFilters.collectionId ? { collection_id: search.searchFilters.collectionId } : {}),
        ...
        try { setPages(await api.pages.list()); } catch { /* keep existing */ }
      }
    }, 200);
    return () => clearTimeout(timer);
  ...
  const paletteItems = (() => {
    ...
    const items: { type: "page" | "collection" | "action"; id?: string; label: string; subtitle: string; icon: React.ReactNode; action: () => void; shortcut?: string }[] = [];
    ...
    for (const c of collections.filter(x => x.name.toLowerCase().includes(q))) {
      items.push({
        type: "collection", id: c.id, label: c.name, subtitle: `${c.icon || "📁"} Collection`,
        icon: <BookOpenIcon />,
        action: () => { navigate("/"); toggleCollection(c.id); },
      });
    }
    const actions = [
      { label: "New page", subtitle: "Create a new document", icon: <PlusIcon />, action: () => { closePalette(); navigate("/new"); } },
      { label: "New collection", subtitle: "Create a new collection", icon: <FolderPlusIcon />, action: () => { closePalette(); openCreateCol(); } },
      { label: "New template", subtitle: "Save current page as a template", icon: <LayoutTemplateIcon />, action: () => { closePalette(); setTemplatePickerOpen(true); } },
      { label: "Admin panel", subtitle: "Manage users, groups, settings", icon: <ShieldIcon />, action: () => { closePalette(); navigate("/admin"); } },
      { label: "Trash", subtitle: "View deleted pages", icon: <Trash2Icon />, action: () => { closePalette(); navigate("/trash"); } },
      { label: "Favorites", subtitle: "Show starred pages", icon: <StarIcon />, action: () => { closePalette(); navigate("/favorites"); } },
      { label: "Activity", subtitle: "View recent wiki activity", icon: <HistoryIcon />, action: () => { closePalette(); navigate("/activity"); } },
      { label: "Graph view", subtitle: "Visualize page relationships", icon: <Share2Icon />, action: () => { closePalette(); navigate("/graph"); } },
      { label: "API Docs", subtitle: "Open API documentation", icon: <CodeIcon />, action: () => { closePalette(); window.open('/docs', '_blank'); } },
      { label: "Keyboard shortcuts", subtitle: "View all keyboard shortcuts", icon: <KeyboardIcon />, action: () => { closePalette(); setShortcutsOpen(true); } },
      { label: "Toggle dark mode", subtitle: `Switch to ${theme === "dark" ? "light" : "dark"} theme`, icon: theme === "dark" ? <SunIcon /> : <MoonIcon />, action: () => { closePalette(); setTheme(theme === "dark" ? "light" : "dark"); } },
    ];
    for (const a of actions) {
      if (a.label.toLowerCase().includes(q) || q === "") items.push({ type: "action", label: a.label, subtitle: a.subtitle, icon: a.icon, action: a.action });
    }
  ...
  return {
    ...
    pages, pagesByCollection, collectionTree,
    allUsers, setAllUsers,
    loading, userId,
    theme, setTheme,
    shortcutsOpen, setShortcutsOpen,
    aiAssistantOpen, setAiAssistantOpen,
    favoritePages, expandedCollections, pageLimits,
    setExpandedCollections, setPageLimits,
    PAGE_LIMIT,
    ...
    // Collection
    colDialogOpen, setColDialogOpen,
    editingCol, colName, setColName,
    colDesc, setColDesc, colIcon, setColIcon,
    colColor, setColColor, colSortMode, setColSortMode,
    colAutoApply, setColAutoApply,
    collectionSortModes,
    openCreateCol, openEditCol, saveCollection, deleteCollection,

    // Trash
    trashPages, trashLoading,
    loadTrashPage, restorePage, permanentDelete, emptyTrash,
    ...
    openShareDialog, createShare, deleteShare,
    updateShareBranding, openBrandingEditor,

    // Templates
    templatePickerOpen, setTemplatePickerOpen,
    templateModalOpen, setTemplateModalOpen,
    templates, newPageTitle, setNewPageTitle,
    selectedTemplate, setSelectedTemplate,
    openTemplates, createFromTemplate,
    ...
    paletteItems, executePalette, closePalette,

    // Computed
    isActive, currentPageId, currentPageTitle,
    toggleCollection, collectionLabel,

    // Export
    handleExportPageMD, handleExportPageHTML, handleDuplicatePage,
    refreshData,
  };
}
