...
export function useImportExport(userId: string | null, onRefresh: () => void) {
  ...
  const handleImportMD = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast({ type: "success", title: "Imported", message: `"${title}" imported from Markdown`, duration: 4000 });
    } catch (err) {
  ...
  const handleImportNotion = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    try {
      ...
      if (file.name.endsWith(".html") || file.name.endsWith(".htm")) {
        ...
        await api.pages.create(title, JSON.stringify(doc), "", "", userId || "anonymous");
        showToast({ type: "success", title: "Imported", message: `"${title}" imported from HTML`, duration: 4000 });
        return;
      }
      ...
      zip.forEach((path, entry) => {
        if (!entry.dir) {
          ...
          if (filename.endsWith(".md")) {
            contentEntries.push({ path, name: filename.replace(/\.md$/i, ""), dir: parts.join("/"), ext: ".md" });
          } else if (filename.endsWith(".html") || filename.endsWith(".htm")) {
            contentEntries.push({ path, name: filename.replace(/\.html?$/i, ""), dir: parts.join("/"), ext: ".html" });
          }
        }
      });

      if (contentEntries.length === 0) {
        showToast({ type: "error", title: "No pages found", message: "No Markdown or HTML files found", duration: 5000 });
        return;
      }
      ...
      showToast({ type: "success", title: "Wiki imported", message: `Created ${created} pages`, duration: 4000 });
      onRefresh();
    } catch (err) {
      showToast({ type: "error", title: "Import failed", message: String(err), duration: 5000 });
    } finally {
  ...
  const handleImportConfluence = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    try {
      ...
      showToast({ type: "success", title: "Confluence import complete", message: `Created ${result.pages_created || 0} pages`, duration: 4000 });
      onRefresh();
    } catch (err) {
      showToast({ type: "error", title: "Confluence import failed", message: String(err), duration: 5000 });
    } finally {
  ...
  return {
    importing, importingNotion, importingConfluence,
    importRef, notionImportRef, confluenceImportRef,
    handleImportMD, handleImportNotion, handleImportConfluence,
  };
}
