...
export function useShareManagement(userId: string | null) {
  const [shareDialog, setShareDialog] = useState<{ pageId: string; pageTitle: string } | null>(
    null,
  );
  ...
  const updateShareBranding = useCallback(
  ...
  const openBrandingEditor = useCallback(
    (share: { id: string; brand_title: string | null; brand_logo_url: string | null }) => {
