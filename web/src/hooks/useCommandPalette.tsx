...
import {
  FileText, BookOpen, Plus, FolderPlus, LayoutTemplate, Shield, Trash2,
  Star, History, Share2, Code, Keyboard, Sun, Moon,
} from "lucide-react";
...
export function useCommandPalette(
  ...
  const paletteItems = useMemo(() => {
    ...
    for (const a of actions) {
      if (a.label.toLowerCase().includes(q) || q === "") results.push({ type: "action" as const, label: a.label, subtitle: a.subtitle, icon: a.icon, action: a.action });
    }
    return results.slice(0, 15);
  }, [paletteQuery, pages, collections, navigate, theme, openCreateCol, toggleCollection, closePalette, setTemplatePickerOpen, setShortcutsOpen, setTheme]);
