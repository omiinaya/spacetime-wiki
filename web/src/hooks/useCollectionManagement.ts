...
export function useCollectionManagement(
  ...
  const openEditCol = useCallback(async (col: Collection) => {
    setEditingCol(col);
    setColName(col.name);
  ...
  }, [colName, editingCol, colDesc, colIcon, colColor, colSortMode, colAutoApply, userId, saveCollectionSortMode, refreshData, addToast]);
