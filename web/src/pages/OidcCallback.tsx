...
export default function OidcCallback() {
  ...
  useEffect(() => {
    ...
    if (error || !code) {
      setStatus(`Authentication failed: ${error || 'No authorization code'}`);
      return;
    }
    ...
    if (!providerId) {
      setStatus('No OIDC provider configured');
      return;
    }
    ...
    (async () => {
      try {
        ...
        if (!provider) {
          setStatus('OIDC provider not found');
          return;
        }
        ...
        if (tokens.error) {
          setStatus(`Token error: ${tokens.error_description || tokens.error}`);
          return;
        }
        ...
        if (!email) {
          setStatus('Could not get email from provider');
          return;
        }
