...
export default function LoginView() {
  ...
  const handleOidcSignIn = (provider: OidcProvider) => {
    ...
    crypto.subtle.digest("SHA-256", new TextEncoder().encode(cv)).then(hash => {
      const cc = btoa(String.fromCharCode(...new Uint8Array(hash))).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
      ...
      fetch(`${issuer}/.well-known/openid-configuration`).then(r => r.json()).then(d => {
  ...
  const handleGoogleSignIn = () => {
    ...
    if (!GOOGLE_CLIENT_ID) { setError("Google OAuth not configured."); return; }
    const cv = Array.from(crypto.getRandomValues(new Uint8Array(32)))
      .map(b => "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~"[b % 66]).join("");
    crypto.subtle.digest("SHA-256", new TextEncoder().encode(cv)).then(hash => {
      const cc = btoa(String.fromCharCode(...new Uint8Array(hash))).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
...
<samlp:AuthnRequest xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol"
  ...
  const handleLdapSignIn = async (providerId: string) => {
    setError("");
    if (!ldapUsername.trim() || !ldapPassword.trim()) { setError("LDAP username and password required"); return; }
    setLdapLoading(true);
    try {
      const resp = await fetch(`${window.location.origin}/api/v1/auth/ldap/login`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ provider_id: providerId, username: ldapUsername.trim(), password: ldapPassword }),
      });
  ...
  const handleMfaVerify = async () => {
    ...
    try {
      if (mfaBackupMode) await api.mfa.verifyBackupCode(pendingUserId, mfaBackupCode);
        ...
        if (isNaN(code) || code < 100000 || code > 999999) { setMfaError("Enter a valid 6-digit code"); return; }
  ...
  const handlePasskeySignIn = async () => {
    setError("");
    try {
      const resp = await fetch(`${window.location.origin}/api/v1/webauthn/auth/begin?email=${encodeURIComponent(email || "")}`);
      ...
      const verifyResp = await fetch(`${window.location.origin}/api/v1/webauthn/auth/complete`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id: credential.id, rawId: arrayBufferToBase64Url(credential.rawId), type: credential.type,
          response: {
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {mfaRequired ? "Enter the verification code from your authenticator app." : (isRegister ? "Join your team's knowledge base." : "Welcome back.")}
          </p>
        </div>
        ...
        {mfaRequired ? (
          <div className="space-y-4">
            ...
            <button onClick={handleMfaVerify} disabled={mfaBackupMode ? mfaBackupCode.length < 4 : mfaCode.length !== 6}
              className="w-full h-9 rounded-md bg-primary text-white text-sm font-medium hover:bg-primary/90 disabled:opacity-50">Verify</button>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {isRegister && (
              <div>
                <label htmlFor="login-name" className="block text-xs font-medium mb-1">Name</label>
            ...
            <div>
              <label htmlFor="login-email" className="block text-xs font-medium mb-1">Email</label>
              <input id="login-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                className="w-full h-9 px-3 rounded-md border border-border bg-card text-sm focus:outline-hidden focus:ring-1 focus:ring-primary/50" required />
            </div>
            <div>
              <label htmlFor="login-password" className="block text-xs font-medium mb-1">Password</label>
