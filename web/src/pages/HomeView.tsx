...
import { Loader2, FileText, Eye, Plus, Upload, Library, Keyboard, Edit3, Search, BookOpen, MessageSquare, Shield, Download, Star, LayoutTemplate } from "lucide-react";
    ...
    return (
      <div className="p-4 md:p-6 lg:p-8 max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-8">
          ...
          <button onClick={() => document.dispatchEvent(new KeyboardEvent("keydown", { key: "?" }))}
            className="flex flex-col items-center gap-2 p-6 rounded-xl border border-border bg-card hover:bg-secondary hover:border-primary/30 transition-all group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {[
              ...
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg border border-border bg-card hover:bg-secondary transition-colors">
                <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0 text-primary">{feature.icon}</div>
                <div>
        ...
        <div className="text-center pb-8">
          <p className="text-[11px] text-muted-foreground/60">Spacetime Wiki &middot; Built with SpacetimeDB + React + Tiptap</p>
        </div>
      </div>
    );
  }
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-2">
            {recentPages.map((page) => (
              <button key={page.id} onClick={() => navigate(`/page/${page.id}`)}
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-2">
            {trendingPages.map((item) => (
              <button key={item.page_id} onClick={() => navigate(`/page/${item.page_id}`)}
                className="flex items-center gap-3 p-3 rounded-lg border border-border bg-card hover:bg-secondary transition-colors text-left">
