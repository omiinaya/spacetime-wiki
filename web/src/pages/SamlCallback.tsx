  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!samlResponse) { setStatus("No SAMLResponse received."); return; }
        ...
        try {
          ...
          try { decodedXml = pako.inflate(bytes, { to: "string" }) as string; }
          catch { decodedXml = new TextDecoder().decode(bytes); }
        } catch { setStatus("Failed to decode SAMLResponse."); return; }
        ...
        if (!providerId) { setStatus("Could not determine SAML provider."); return; }
        ...
        if (!provider) { setStatus("SAML provider not found."); return; }
        let mapping: Record<string, string> = { email: "email", name: "name" };
        try { mapping = JSON.parse(provider.attribute_mapping); } catch {}

        const email = attributes[mapping.email] || attributes["email"] || attributes["nameId"] || "";
        const displayName = attributes[mapping.name] || attributes["name"] || email?.split("@")[0] || "User";
        if (!email) { setStatus("Could not determine email."); return; }
