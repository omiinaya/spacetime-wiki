  ...
  return (
    <div className="p-2 bg-gray-800 border border-gray-700 rounded-lg shadow-xl min-w-[160px]">
      {choices.map((choice) => (
        ...
        >
          {multi && <span className="mr-2">{localSelected.includes(choice) ? '✓' : ''}</span>}
          {choice}
        </button>
      ))}
      {multi && (
...
const DatabaseBaseNodeView: React.FC<NodeViewProps> = ({
      ...
      <table className="w-full border-collapse">
        ...
        <thead>
          <tr className="border-b border-gray-700">
            <th className="text-left px-3 py-2 text-xs font-medium text-gray-400 uppercase tracking-wider w-8">
              #
            </th>
            {columns.map((col) => (
              <th
                ...
                >
                  {editingCell?.rowId === row.id && editingCell?.colId === col.id ? (
                    ...
                    <div className="min-h-[24px] flex items-center group/cell">
                      ...
                      <span className="opacity-0 group-hover/cell:opacity-40 ml-2 text-gray-600 text-xs">
                        ✎
                      </span>
                    </div>
                  )}
      ...
      return (
        <div className="p-4 text-center text-gray-500 text-sm">
          ...
          <p className="text-xs text-gray-600">
            Add a column with type "Select" and name it "Status" to enable kanban grouping.
          </p>
          {renderTableView()}
        </div>
      );
    ...
    return (
      <div className="flex gap-4 overflow-x-auto pb-4">
        {sortedGroups.map(([status, groupRows]) => (
          <div
            key={status}
            className="shrink-0 w-72 bg-gray-900/80 border border-gray-800 rounded-xl"
          >
            {/* Column header */}
  ...
  return (
    <div
      className={`my-4 bg-gray-900/40 border border-gray-800/60 rounded-xl overflow-hidden ${selected ? 'ring-2 ring-indigo-500/50' : ''}`}
    >
      {/* Toolbar */}
      <div className="flex items-center justify-between px-4 py-2.5 bg-gray-900/80 border-b border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] text-gray-600 bg-gray-800 px-2 py-0.5 rounded-full uppercase">
            {viewType}
          </span>
        </div>
      ...
      {/* Content */}
      <div className="p-4">{viewType === 'kanban' ? renderKanbanView() : renderTableView()}</div>
