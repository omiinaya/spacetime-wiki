...
export const SyncedBlockExtension = Node.create<SyncedBlockOptions>({
  ...
  renderHTML({ node, HTMLAttributes }) {
    return [
      ...
      [
        "div",
        { class: "synced-block-header flex items-center gap-2 mb-2 pb-1 border-b border-blue-300/20 text-xs font-medium text-blue-400/70" },
        ["span", {}, "🔄 Synced block:"],
        ["span", { class: "font-semibold text-blue-300/90" }, node.attrs.blockTitle || node.attrs.blockId],
      ],
      [
        "div",
...
const SyncedBlockNodeView: React.FC<{ node: any }> = ({ node }) => {
  ...
  useEffect(() => {
    if (node.attrs.blockId && !node.attrs.content) {
      api.syncedBlocks.get(node.attrs.blockId).then((data) => {
        if (data) {
          setBlockData(data);
          try {
  ...
  const content = node.attrs.content || (blockData ? (() => { try { return JSON.parse(blockData.content); } catch { return null; } })() : null);
      ...
      <div className="synced-block-content prose prose-invert prose-sm max-w-none">
        {loading ? (
          ...
          renderProseMirrorContent(content).map((el, i) => {
            ...
            return <Tag key={i} {...attrs}>{children}</Tag>;
          })
        ) : (
