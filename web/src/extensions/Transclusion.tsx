  ...
  renderHTML({ node, HTMLAttributes }) {
    return [
      ...
      [
        ...
        {
          class:
            'transclusion-header flex items-center gap-2 mb-2 pb-1 border-b border-primary/10 text-xs font-medium text-primary/60',
        },
        ['span', {}, '📄 Included from:'],
        [
          'span',
          { class: 'font-semibold text-primary/80' },
          node.attrs.pageTitle || node.attrs.pageId,
        ],
      ],
      [
        'div',
...
function renderProseMirrorContent(node: PMNode): unknown[] {
  ...
  if (node.type === 'text') {
    ...
    if (node.marks) {
      for (const mark of node.marks) {
        ...
        if (mark.type === 'link')
          text = `<a href="${(mark.attrs as { href?: string })?.href || ''}" class="text-primary underline">${text}</a>`;
      }
    }
  ...
  if (node.type === 'heading') {
    ...
    return [
      [
        `h${level}`,
        { class: `text-${['', 'xl', 'lg', 'base'][level] || 'base'} font-semibold mt-2 mb-1` },
        text,
      ],
    ];
  }
  ...
  if (node.type === 'codeBlock') {
    const text = node.content?.map((c: PMNode) => c.text || '').join('') || '';
    return [
      ['pre', { class: 'bg-muted p-2 rounded text-xs overflow-x-auto my-1' }, ['code', {}, text]],
    ];
  }
  if (node.type === 'blockquote') {
    ...
    return [
      [
        'blockquote',
        { class: 'border-l-2 border-muted-foreground/20 pl-3 my-1 italic' },
        ...children,
      ],
    ];
  }
 ...
 */
function TransclusionNodeView({
  node,
}: {
  node: { attrs: { pageTitle: string; pageId: string; content: PMNode | null } };
}) {
...
function TransclusionContentRenderer({ content }: { content: PMNode }) {
    ...
    return (
      <>
        {content.content.map((c: PMNode, i: number) => (
          <TransclusionContentRenderer key={i} content={c} />
        ))}
      </>
    );
  }
  ...
  if (content.type === 'text') {
    ...
    if (content.marks) {
      ...
      for (const mark of content.marks) {
        ...
        if (mark.type === 'code')
          return (
            <code key={text} className="bg-muted px-1 rounded text-xs">
              {text}
            </code>
          );
      ...
      if (hasLink) {
        return (
          <a key={text} href={href} className="text-primary underline">
            {text}
          </a>
        );
      }
      ...
      <ul className="list-disc pl-4 my-1">
        {content.content?.map((c: PMNode, i: number) => (
          <li key={i} className="mb-0.5">
            {c.content?.map((cc: PMNode, j: number) => (
              <TransclusionContentRenderer key={j} content={cc} />
            ))}
          </li>
        ))}
      </ul>
  ...
  if (content.type === 'image') {
    return (
      <img
        src={(content.attrs as { src?: string })?.src || ''}
        alt={(content.attrs as { alt?: string })?.alt || ''}
      ...
      <ul className="list-none pl-0 my-1">
        {content.content?.map((c: PMNode, i: number) => (
          <li key={i} className="flex items-start gap-2 mb-0.5">
            <input
              type="checkbox"
              checked={(c.attrs as { checked?: boolean })?.checked || false}
              disabled
              className="mt-1"
            />
            <span>
