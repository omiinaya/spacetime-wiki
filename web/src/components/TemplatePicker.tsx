...
export function TemplatePicker({ open, onClose, collections, userId, navigate }: TemplatePickerProps) {
  ...
  useEffect(() => {
    ...
    api.pages.listTemplates().then(setTemplates).catch(() => {}).finally(() => setLoading(false));
  }, [open, collections]);
