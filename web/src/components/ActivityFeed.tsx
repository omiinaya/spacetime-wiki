...
export function getEventColor(eventType: string): string {
  ...
  if (eventType.includes('archive') || eventType.includes('status_change'))
  ...
  if (error) {
    return (
      <div className="text-center py-4 text-red-400">
        <p>{error}</p>
        <button onClick={loadEvents} className="mt-2 text-sm text-blue-400 hover:underline">
          Try again
        </button>
      </div>
    );
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm">
                {event.actor_id && <span className="font-medium text-gray-200">{actorName}</span>}{' '}
                <span className="text-gray-400">{getEventLabel(event.event_type)}</span>{' '}
                {event.target_name && (
                  <span className="font-medium text-gray-200 truncate">"{event.target_name}"</span>
                )}
              </div>
              <div className="text-xs text-gray-500 mt-0.5">{timeAgo(event.created_at)}</div>
            </div>
