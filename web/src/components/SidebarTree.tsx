    ...
    return tree.map((col) => {
      ...
      return (
        <div key={col.id} className="mb-0.5">
          ...
          {expanded && (
            <>
              {(col as unknown).children.length > 0 &&
                renderColTree((col as unknown).children as unknown, depth + 1)}
                ...
                >
                  ...
                  >
                    ...
                    {searchQuery && page.text_content && (
                      <span className="block text-[11px] text-muted-foreground/60 mt-0.5 max-w-full leading-relaxed">
                        {(() => {
                          ...
                          if (start > 0)
                            parts.push(
                              <span key="lead" className="opacity-50">
                                …
                              </span>,
                            );
                          while (matchIdx >= 0) {
                            if (matchIdx > cursor)
                              parts.push(
                                <span key={`t-${cursor}`}>{snippet.slice(cursor, matchIdx)}</span>,
                              );
                            parts.push(
                              <mark
                                key={`m-${matchIdx}`}
                                className="bg-yellow-500/30 text-foreground rounded-xs px-0.5"
                              >
                          ...
                          if (end < text.length)
                            parts.push(
                              <span key="trail" className="opacity-50">
                                …
                              </span>,
                            );
                          return parts;
                    ...
                    {page.status === 'archived' && (
                      <span className="ml-auto text-[10px] px-1 py-0.5 rounded bg-muted text-muted-foreground shrink-0">
                        Archived
                      </span>
                    )}
                    {page.is_template && (
                      <span className="ml-auto text-[10px] px-1 py-0.5 rounded bg-purple-500/10 text-purple-400 shrink-0">
                        Template
                      </span>
                    )}
                  </button>
              ...
              {colPages.length > (pageLimits[col.id] || PAGE_LIMIT) && (
                <button
                  onClick={() =>
                    setPageLimits((prev) => ({
                      ...prev,
                      [col.id]: (prev[col.id] || PAGE_LIMIT) + PAGE_LIMIT,
                    }))
                  }
                ...
                >
                  <ChevronDown className="h-3 w-3 shrink-0" /> Show{' '}
                  {colPages.length - (pageLimits[col.id] || PAGE_LIMIT)} more
                </button>
              )}
            </>
          )}
    ...
    <>
      ...
      {favoritePages.length > 0 && (
        <div className="px-2 py-1 mb-1">
          <p className="text-[10px] text-muted-foreground/60 font-semibold uppercase tracking-wider px-2 mb-1">
            Favorites
          </p>
          {favoritePages.map((p) => (
            <button
              ...
              className={cn(
                'w-full flex items-center gap-2 pl-2 pr-2 py-1 rounded-md text-xs transition-colors text-left',
                isActive(p.id)
                  ? 'bg-primary/10 text-primary font-medium'
                  : 'text-muted-foreground hover:text-foreground hover:bg-muted/50',
              )}
            ...
            {pagesByCollection['uncategorized']?.length > 0 && (
              <div className="mb-0.5">
                ...
                >
                  ...
                  <span className="text-[10px] text-muted-foreground/50 ml-auto">
                    {pagesByCollection['uncategorized'].length}
                  </span>
                </button>
                {expandedCollections.has('uncategorized') &&
                  pagesByCollection['uncategorized']
                    .slice(0, pageLimits['uncategorized'] || PAGE_LIMIT)
                    .map((page: unknown) => (
                      <div
                        key={page.id}
                        draggable
                ...
                {(pagesByCollection['uncategorized']?.length || 0) >
                  (pageLimits['uncategorized'] || PAGE_LIMIT) && (
                  <button
                    onClick={() =>
                      setPageLimits((prev) => ({
                        ...prev,
                        uncategorized: (prev['uncategorized'] || PAGE_LIMIT) + PAGE_LIMIT,
                      }))
                    }
                    className="w-full flex items-center gap-2 pl-8 pr-2 py-1 text-[10px] text-muted-foreground hover:text-foreground hover:bg-muted/30 rounded-md transition-colors text-left"
                  >
        ...
        >
          ...
          >
            <h3 className="text-sm font-semibold mb-3">
              Add tag to {selectedPageIds.size} page(s)
            </h3>
