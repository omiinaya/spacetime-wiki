...
export function ShareDialog({
  pageTitle, sharePassword, onPasswordChange, shareDays, onDaysChange,
  shareUrl, shareLinks, onCreateShare, onDeleteShare,
  editBrandShareId, onEditBrandShareId, editBrandTitle, onEditBrandTitle,
  editBrandLogoUrl, onEditBrandLogoUrl, onUpdateBranding, onClose,
}: ShareDialogProps) {
  return (
    <div className="dialog-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="dialog-container w-full max-w-sm mx-4 p-5 rounded-xl border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold flex items-center gap-2"><Link2 className="h-4 w-4 text-primary" /> Share &quot;{pageTitle}&quot;</h3>
          <button onClick={onClose} className="p-1 rounded hover:bg-muted"><X className="h-4 w-4" /></button>
        </div>
        <div className="space-y-3">
          <div>
            <label className="text-[10px] text-muted-foreground/60 mb-1 block">Password (optional)</label>
            <input type="text" value={sharePassword} onChange={(e) => onPasswordChange(e.target.value)}
          ...
          <div>
            ...
            <input type="number" value={shareDays} onChange={(e) => onDaysChange(parseInt(e.target.value) || 0)} min={0}
          ...
          <button onClick={onCreateShare} className="w-full h-8 rounded-md text-xs font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition-colors">
            Create share link
          </button>
          {shareUrl && (
              ...
              {shareLinks.map(s => (
                <div key={s.id}>
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <button onClick={() => onEditBrandShareId(s.id)} className="text-[10px] text-purple-400 hover:text-purple-300" title="Customize branding">🎨</button>
                    <button onClick={() => onDeleteShare(s.id)} className="text-red-400 hover:text-red-300 text-[10px]">×</button>
                  </div>
                  {editBrandShareId === s.id && (
                    <div className="ml-4 mt-1 p-2 rounded-md bg-muted/30 border border-border space-y-1.5">
                      <input type="text" value={editBrandTitle} onChange={(e) => onEditBrandTitle(e.target.value)}
                      ...
                      <input type="text" value={editBrandLogoUrl} onChange={(e) => onEditBrandLogoUrl(e.target.value)}
                        placeholder="Logo URL"
                        className="w-full h-7 px-2 rounded border border-border bg-[#0a0a0a] text-[10px] text-foreground placeholder:text-muted-foreground/50 focus:outline-hidden focus:ring-1 focus:ring-primary/50" />
                      <div className="flex gap-1.5">
                        <button onClick={() => onUpdateBranding(s.id)}
                          className="flex-1 h-6 rounded text-[10px] font-medium bg-purple-600 text-white hover:bg-purple-500 transition-colors">Save branding</button>
                        <button onClick={() => onEditBrandShareId(null)}
                          className="h-6 px-2 rounded text-[10px] text-muted-foreground hover:text-foreground transition-colors">Cancel</button>
                      </div>
                    </div>
                  )}
                </div>
