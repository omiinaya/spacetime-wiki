...
export function CommandPalette({ open, query, onQueryChange, index, items, onExecute, onClose }: CommandPaletteProps) {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] bg-black/60" onClick={onClose}>
      <div className="w-full max-w-xl rounded-xl border border-border bg-card shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="max-h-[50vh] overflow-y-auto">
          ...
          {items.map((item, i) => (
            <button key={item.type + (item.id || item.label)}
              onClick={() => onExecute(i)}
              className={cn("w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors", i === index ? "bg-primary/10" : "hover:bg-muted/50")}>
