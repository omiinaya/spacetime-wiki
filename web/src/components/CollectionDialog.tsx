...
export function CollectionDialog({
  open, onClose, editingCol,
  colName, onColNameChange,
  colDesc, onColDescChange,
  colIcon, onColIconChange,
  colColor, onColColorChange,
  colSortMode, onColSortModeChange,
  colAutoApply, onColAutoApplyChange,
  onSave, saveDisabled,
      ...
      <div className="dialog-container w-full max-w-sm p-5 rounded-xl border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div className="flex gap-2">
            <input type="text" value={colIcon} onChange={(e) => onColIconChange(e.target.value)}
            ...
            <input type="text" value={colName} onChange={(e) => onColNameChange(e.target.value)}
          ...
          <input type="text" value={colDesc} onChange={(e) => onColDescChange(e.target.value)}
          ...
          <input type="text" value={colColor} onChange={(e) => onColColorChange(e.target.value)}
          ...
          {editingCol && (
            <div>
              ...
              <select value={colSortMode} onChange={(e) => onColSortModeChange(e.target.value)}
              ...
              {colSortMode !== "manual" && (
                <label className="flex items-center gap-2 mt-2 cursor-pointer">
                  <input type="checkbox" checked={colAutoApply} onChange={(e) => onColAutoApplyChange(e.target.checked)}
          ...
          <div className="flex gap-2 justify-end pt-2">
            <button onClick={onClose} className="h-8 px-3 rounded-md text-xs text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
              Cancel
            </button>
            <button onClick={onSave} disabled={saveDisabled} className="h-8 px-4 rounded-md text-xs font-medium bg-primary text-white hover:bg-primary/90 disabled:opacity-50 transition-colors">
