...
const COLORS: string[] = [
  ...
  "#6366f1","#d946ef","#00FFFF","#a855f7","#eab308",
];
...
export default function GraphView() {
        ...
        active.forEach((page) => {
          ...
          while ((m = re.exec(page.text_content)) !== null) {
            ...
            if (tid !== page.id && nodeIds.has(tid)) {
              const exists = gLinks.some(l => l.source === page.id && l.target === tid && l.type === "backlink");
  ...
  useEffect(() => {
    ...
    // Links
    const linkEls = svg.append("g").selectAll("line").data(activeLinks).join("line")
      .attr("stroke", d => d.type === "parent-child" ? "#60a5fa" : d.type === "backlink" ? "#f59e0b" : "#888")
    ...
    // Nodes
    const nodeEls = svg.append("g").selectAll("g").data(graphNodes).join("g").attr("cursor", "pointer");
    ...
    nodeEls.append("text")
      ...
      .attr("dx", d => d.isCollection ? 22 : 14).attr("dy", 4)
    ...
    nodeEls.filter(d => d.isCollection).append("text")
      .text(d => d.icon || "\uD83D\uDCC1").attr("text-anchor", "middle").attr("dy", 4).attr("font-size", 12)
      .style("pointer-events", "none");

    nodeEls.on("mouseenter", (_e, d) => setHoveredNode(d))
  ...
  const handleZoomIn = useCallback(() => {
    ...
    if (g) { const z = Math.min(zoom * 1.3, 4); g.setAttribute("transform", `scale(${z})`); setZoom(z); }
  }, [zoom]);

  const handleZoomOut = useCallback(() => {
    const g = svgRef.current?.parentElement?.querySelector("g");
    if (g) { const z = Math.max(zoom / 1.3, 0.2); g.setAttribute("transform", `scale(${z})`); setZoom(z); }
  }, [zoom]);

  const handleResetZoom = useCallback(() => {
    const g = svgRef.current?.parentElement?.querySelector("g");
    if (g) { g.setAttribute("transform", "scale(1)"); setZoom(1); }
  }, []);
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-full min-h-[400px]">
        <div className="text-sm text-red-400 bg-red-500/10 px-4 py-2 rounded-md border border-red-500/30">{error}</div>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 py-3 border-b border-border flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">{graphNodes.length} nodes &middot; {graphLinks.length} links</span>
        </div>
        <div className="flex items-center gap-2">
          <label className="flex items-center gap-1 text-xs text-muted-foreground cursor-pointer">
            <input type="checkbox" checked={showCollections} onChange={e => setShowCollections(e.target.checked)} className="accent-primary" />
            C
          </label>
          <label className="flex items-center gap-1 text-xs text-muted-foreground cursor-pointer">
            <input type="checkbox" checked={showParentChild} onChange={e => setShowParentChild(e.target.checked)} className="accent-primary" />
            P/C
          </label>
          <label className="flex items-center gap-1 text-xs text-muted-foreground cursor-pointer">
            <input type="checkbox" checked={showBacklinks} onChange={e => setShowBacklinks(e.target.checked)} className="accent-primary" />
            BL
          </label>
          <span className="w-px h-4 bg-border mx-1" />
          <button onClick={handleZoomIn} className="p-1.5 rounded text-muted-foreground hover:text-foreground hover:bg-muted" title="Zoom in"><ZoomIn className="h-4 w-4" /></button>
          <button onClick={handleZoomOut} className="p-1.5 rounded text-muted-foreground hover:text-foreground hover:bg-muted" title="Zoom out"><ZoomOut className="h-4 w-4" /></button>
          <button onClick={handleResetZoom} className="p-1.5 rounded text-muted-foreground hover:text-foreground hover:bg-muted" title="Reset"><RotateCcw className="h-4 w-4" /></button>
        </div>
      </div>
      ...
      <div ref={containerRef} className="flex-1 relative overflow-hidden bg-[#0a0a0a]">
          ...
          <div className="absolute bottom-4 left-4 max-w-xs p-3 rounded-lg border border-border bg-card/90 backdrop-blur-xs shadow-xl">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div>
                ...
                <div className="text-[10px] text-muted-foreground">{selectedNode.isCollection ? "Collection" : "Page"} &middot; {selectedNode.collectionName}</div>
              </div>
            </div>
            <div className="flex gap-2 mt-2">
              {!selectedNode.isCollection && (
                <button onClick={() => navigate(`/page/${selectedNode.id}`)}
                  className="h-6 px-2.5 rounded text-[10px] font-medium bg-primary/10 text-primary hover:bg-primary/20">Open page</button>
              )}
              <button onClick={() => setSelectedNode(null)}
                className="h-6 px-2.5 rounded text-[10px] font-medium text-muted-foreground hover:text-foreground hover:bg-muted">Close</button>
            </div>
          </div>
        )}
        ...
        <div className="absolute bottom-4 right-4 p-3 rounded-lg border border-border bg-card/80 backdrop-blur-xs shadow-xl">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2"><span className="w-3 h-0.5 bg-[#888] opacity-30" /><span className="text-[10px] text-muted-foreground">Collection</span></div>
            <div className="flex items-center gap-2"><span className="w-3" style={{ borderTop: "1px dashed #f59e0b", height: 0 }} /><span className="text-[10px] text-muted-foreground">Backlink</span></div>
          </div>
        </div>
      </div>
    </div>
