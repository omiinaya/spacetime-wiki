...
export function OAuthSettings({ userId }: { userId: string | null }) {
  ...
  const openEdit = (p: OauthProvider) => {
    ...
    setOaName(p.name);
    setOaSlug(p.slug);
    setOaType(p.provider_type);
    setOaAuthUrl(p.authorize_url);
    setOaTokenUrl(p.token_url);
    setOaUserUrl(p.userinfo_url);
    setOaScope(p.scope);
    setOaClientId(p.client_id);
    setOaClientSecret('');
    setOaIcon(p.icon);
    setOaAutoReg(p.auto_register);
    setOaDefaultRole(p.default_role);
    setOaActive(p.is_active);
    setError('');
    setDialogOpen(true);
  };
  ...
  const getDefaultUrls = (type: string) => {
    const defaults: Record<
      string,
      { auth: string; token: string; userinfo: string; scope: string; icon: string }
    > = {
      github: {
    ...
    try {
      if (editingId) {
        await api.oauth.updateProvider(editingId, {
          name: oaName.trim(),
          slug: oaSlug.trim() || oaName.trim().toLowerCase().replace(/\s+/g, '-'),
          provider_type: oaType,
          authorize_url: oaAuthUrl.trim(),
          token_url: oaTokenUrl.trim(),
          userinfo_url: oaUserUrl.trim(),
          scope: oaScope,
          client_id: oaClientId.trim(),
          client_secret: oaClientSecret,
          icon: oaIcon || oaType,
          auto_register: oaAutoReg,
          default_role: oaDefaultRole,
          is_active: oaActive,
        });
      } else {
        await api.oauth.addProvider(
          oaName.trim(),
          oaSlug.trim() || oaName.trim().toLowerCase().replace(/\s+/g, '-'),
          oaType,
          oaAuthUrl.trim(),
          oaTokenUrl.trim(),
          oaUserUrl.trim(),
    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
          OAuth Providers (Slack/Discord/GitHub/GitLab)
        </p>
        <button
          onClick={openAdd}
      ...
      <div className="space-y-2">
        ...
        {providers.length === 0 && (
          <div className="py-4 text-center text-xs text-muted-foreground">
            No OAuth providers configured. Add one to enable SSO login with Slack, Discord, GitHub,
            or GitLab.
          </div>
        )}
      </div>
      ...
      <div className="pt-4 mt-4 border-t border-border">
        ...
        <p className="text-[10px] text-muted-foreground/60 leading-relaxed">
          Configure any OAuth 2.0 provider (Slack, Discord, GitHub, GitLab, or custom). Users will
          see a{' '}
          <strong className="text-foreground">
            "Sign in with {providers.find((p) => p.is_active)?.name || 'Provider'}"
          </strong>{' '}
          button on the login page. The callback URL is:{' '}
          <code className="bg-muted px-1 rounded">{window.location.origin}/oauth/callback</code>
        </p>
      </div>
        ...
        >
          ...
          >
            <h3 className="text-sm font-semibold mb-4">
              {editingId ? 'Edit OAuth Provider' : 'Add OAuth Provider'}
            </h3>
            ...
            <div className="space-y-3 max-h-[65vh] overflow-y-auto pr-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={oaSlug}
                    onChange={(e) => setOaSlug(e.target.value)}
                    placeholder="my-github"
                    className="w-full h-8 px-2 rounded-md border border-border bg-[#0a0a0a] text-xs focus:outline-hidden focus:ring-1 focus:ring-primary/50"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={oaUserUrl}
                    onChange={(e) => setOaUserUrl(e.target.value)}
                    className="w-full h-8 px-2 rounded-md border border-border bg-[#0a0a0a] text-xs font-mono focus:outline-hidden focus:ring-1 focus:ring-primary/50"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={oaIcon}
                  onChange={(e) => setOaIcon(e.target.value)}
                  placeholder="github"
                  className="w-full h-8 px-2 rounded-md border border-border bg-[#0a0a0a] text-xs font-mono focus:outline-hidden focus:ring-1 focus:ring-primary/50"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={oaDefaultRole}
                    onChange={(e) => setOaDefaultRole(e.target.value)}
                    className="w-full h-8 px-2 rounded-md border border-border bg-[#0a0a0a] text-xs focus:outline-hidden focus:ring-1 focus:ring-primary/50"
                  >
                ...
                {editingId && (
                  <div>
                    ...
                    <select
                      value={oaActive ? 'true' : 'false'}
                      onChange={(e) => setOaActive(e.target.value === 'true')}
                      className="w-full h-8 px-2 rounded-md border border-border bg-[#0a0a0a] text-xs focus:outline-hidden focus:ring-1 focus:ring-primary/50"
                    >
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={oaAutoReg}
                  onChange={(e) => setOaAutoReg(e.target.checked)}
