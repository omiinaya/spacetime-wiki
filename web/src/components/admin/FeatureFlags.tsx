...
export function FeatureFlags() {
  ...
  const FEATURES: { key: string; label: string; desc: string }[] = [
    {
      key: 'callouts',
      label: 'Callouts / Notices',
      desc: 'Info, warning, tip, and danger callout blocks',
    },
    {
      key: 'mermaid',
      label: 'Mermaid Diagrams',
      desc: 'Flowcharts, sequence diagrams, and Gantt charts',
    },
    { key: 'math', label: 'Math (LaTeX/KaTeX)', desc: 'Inline and block mathematical equations' },
    {
      key: 'embeds',
      label: 'Rich Embeds',
      desc: 'Embed content from YouTube, Figma, CodePen, Spotify, and 30+ providers',
    },
    ...
    {
      key: 'database',
      label: 'Database Bases',
      desc: 'Table and kanban database views inside wiki pages',
    },
    {
      key: 'syncedBlocks',
      label: 'Synced Blocks',
      desc: 'Reusable blocks that sync across pages — edit once, update everywhere',
    },
  ];
  ...
  const toggleFeature = async (key: string) => {
    ...
    try {
      ...
      addToast({
        ...
        message: `"${FEATURES.find((f) => f.key === key)?.label || key}" ${updated[key] ? 'enabled' : 'disabled'}`,
        duration: 2000,
      });
    } catch (e) {
    ...
    <div className="space-y-4">
      ...
      <div className="p-3 rounded-md border border-border bg-muted/10">
        <p className="text-xs text-muted-foreground">
          Toggle editor features on or off. Disabled features will be removed from the editor
          toolbar, slash commands, and keyboard shortcuts. Content created with disabled features
          will still render correctly but cannot be modified.
        </p>
      </div>
      <div className="space-y-1">
        {FEATURES.map((feature) => (
          ...
          >
            ...
            >
              <div
                className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-transform ${flags[feature.key] !== false ? 'translate-x-4' : 'translate-x-0.5'}`}
              />
            </button>
            ...
            <span
              className={`text-[10px] font-medium ${flags[feature.key] !== false ? 'text-emerald-500' : 'text-muted-foreground'}`}
            >
