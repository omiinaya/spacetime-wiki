...
export function LdapSettings({ userId }: { userId: string | null }) {
  ...
  const loadProviders = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to load LDAP providers:', e);
    } finally {
      setLoading(false);
    }
  }, [userId]);
  ...
  const openAdd = () => {
    ...
    setLdapBindPw('');
    setLdapBaseDn('');
    setLdapFilter('(uid={{username}})');
    setLdapUserAttr('uid');
    setLdapEmailAttr('mail');
    setLdapNameAttr('cn');
    setLdapDefaultRole('member');
    setLdapAutoReg(true);
    setLdapActive(true);
    setTestStatus('');
    setTestResult('');
  ...
  const openEdit = (p: LdapProvider) => {
    ...
    setLdapHost(p.host);
    setLdapPort(p.port);
    setLdapSecure(p.is_secure);
    setLdapBindDn(p.bind_dn);
    setLdapBindPw('');
    setLdapBaseDn(p.base_dn);
    setLdapFilter(p.user_filter);
    setLdapUserAttr(p.username_attribute);
    setLdapEmailAttr(p.email_attribute);
    setLdapNameAttr(p.name_attribute);
    setLdapDefaultRole(p.default_role);
    setLdapAutoReg(p.auto_register);
    setLdapActive(p.is_active);
    setTestStatus('');
    setTestResult('');
    setError('');
    setDialogOpen(true);
  ...
  const handleSave = async () => {
    ...
    const provider = {
      name: ldapName.trim(),
      slug: ldapSlug.trim() || ldapName.trim().toLowerCase().replace(/\s+/g, '-'),
      host: ldapHost.trim(),
      port: ldapPort,
      is_secure: ldapSecure,
      bind_dn: ldapBindDn,
      bind_password: ldapBindPw,
      base_dn: ldapBaseDn.trim(),
      user_filter: ldapFilter,
      username_attribute: ldapUserAttr,
      email_attribute: ldapEmailAttr,
      name_attribute: ldapNameAttr,
      default_role: ldapDefaultRole,
    ...
    <div>
      ...
      <div className="pt-4 mt-4 border-t border-border">
        ...
        <p className="text-[10px] text-muted-foreground/60 leading-relaxed">
          Configure an LDAP server (OpenLDAP, Active Directory, FreeIPA, etc.). Users will see a{' '}
          <strong className="text-foreground">"Sign in with LDAP"</strong> section on the login
          page. The system binds to LDAP using the service account, searches for the user, then
          authenticates them with their LDAP password.
        </p>
      </div>
      ...
      {dialogOpen && (
        ...
        >
          <div
            className="dialog-container w-full max-w-lg mx-4 p-5 rounded-xl border border-border bg-card shadow-2xl max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-sm font-semibold mb-4">
              {editingId ? 'Edit LDAP Provider' : 'Add LDAP Provider'}
            </h3>
            ...
            <div className="space-y-3 max-h-[65vh] overflow-y-auto pr-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={ldapSlug}
                    onChange={(e) => setLdapSlug(e.target.value)}
                    placeholder="company-ldap"
                    className="w-full h-8 px-2 rounded-md border border-border bg-[#0a0a0a] text-xs focus:outline-hidden focus:ring-1 focus:ring-primary/50"
                  />
                </div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={ldapSecure}
                  onChange={(e) => setLdapSecure(e.target.checked)}
              ...
              <div>
                <label className="text-[10px] text-muted-foreground/60 mb-1 block">
                  Bind DN (service account, leave empty for anonymous bind)
                </label>
                <input
                  type="text"
                  value={ldapBindDn}
              ...
              <div>
                <label className="text-[10px] text-muted-foreground/60 mb-1 block">
                  User Filter (<code className="bg-muted px-1">{'{{username}}'}</code> is replaced
                  with login input)
                </label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={ldapDefaultRole}
                    onChange={(e) => setLdapDefaultRole(e.target.value)}
                    className="w-full h-8 px-2 rounded-md border border-border bg-[#0a0a0a] text-xs focus:outline-hidden focus:ring-1 focus:ring-primary/50"
                  >
                ...
                {editingId && (
                  <div>
                    ...
                    <select
                      value={ldapActive ? 'true' : 'false'}
                      onChange={(e) => setLdapActive(e.target.value === 'true')}
                      className="w-full h-8 px-2 rounded-md border border-border bg-[#0a0a0a] text-xs focus:outline-hidden focus:ring-1 focus:ring-primary/50"
                    >
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={ldapAutoReg}
                  onChange={(e) => setLdapAutoReg(e.target.checked)}
                  className="rounded border-border"
                />
                <span className="text-[10px] text-muted-foreground/80">
                  Auto-register new users on first LDAP login
                </span>
              </label>
            </div>
            ...
            {testStatus && (
              <div className="mt-3 px-3 py-2 rounded-md text-xs bg-blue-500/10 text-blue-400">
                {testStatus}
              </div>
            )}
            {testResult && (
              <div className="mt-2 px-3 py-2 rounded-md text-xs bg-green-500/10 text-green-400">
                {testResult}
              </div>
            )}
