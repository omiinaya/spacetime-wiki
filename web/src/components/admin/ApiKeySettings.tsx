  ...
  return (
    <div className="space-y-2">
      {keys.map((k) => (
        <div
          key={k.id}
          className="flex items-center gap-2 px-3 py-1.5 rounded-md border border-border text-xs"
        >
      ...
      {newKey && (
        <div className="p-2 rounded-md bg-green-500/10 border border-green-500/20">
          <p className="text-[10px] text-green-500 font-medium mb-1">
      ...
      <div className="flex gap-2">
        ...
        <input
          type="number"
          value={keyExpiry}
          onChange={(e) => setKeyExpiry(parseInt(e.target.value) || 0)}
          min={0}
          placeholder="Days"
