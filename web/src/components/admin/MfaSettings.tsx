    ...
    <div>
      ...
      <div className="p-3 rounded-md border border-border mb-4">
        <div className="flex items-center gap-2">
          <div
            className={`h-2 w-2 rounded-full ${method?.is_enabled ? 'bg-green-500' : 'bg-muted-foreground/30'}`}
          />
        ...
        {method?.is_enabled && (
          <div className="mt-2 text-[10px] text-muted-foreground/60">
            <span className="font-medium text-foreground/80">Method:</span> TOTP (Time-based
            One-Time Password) · {savedBackupCodes.length} unused backup codes
          </div>
        )}
      </div>
      ...
      {setupOpen && (
        ...
        >
          <div
            className="dialog-container w-full max-w-md mx-4 p-5 rounded-xl border border-border bg-card shadow-2xl max-h-[90vh] overflow-y-auto"
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground/60 text-center">
                  Or manually enter:{' '}
                  <code className="bg-muted px-1 rounded text-[10px] font-mono">{totpSecret}</code>
                </p>
              </div>
              ...
              {backupCodes.length > 0 && (
                <div className="p-3 rounded-md bg-yellow-500/10 border border-yellow-500/20">
                  ...
                  <p className="text-[10px] text-yellow-400/70 mb-2">
                    ...
                    app. Store them somewhere safe.
                  </p>
                  <div className="grid grid-cols-2 gap-1">
                    {backupCodes.map((code, i) => (
                      <code
                        key={i}
                        className="text-xs font-mono bg-black/20 px-2 py-1 rounded text-yellow-300"
                      >
                        {code}
                      </code>
                    ))}
