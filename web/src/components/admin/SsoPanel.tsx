...
export function SsoPanel({ addToast }: SsoPanelProps) {
  ...
  const [samlNameIdFmt, setSamlNameIdFmt] = useState(
          ...
          prev.map((p) =>
            p.id === editingSaml.id
              ? {
                  ...
                  sso_url: samlSsoUrl,
                  cert: samlCert,
                  name_id_format: samlNameIdFmt,
                  attribute_mapping: samlAttrMapping,
                  auto_register: samlAutoRegister,
                }
              : p,
          ),
        );
  ...
  const resetSamlForm = () => {
    ...
    setSamlSsoUrl('');
    setSamlCert('');
    setSamlNameIdFmt('urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress');
    setSamlAttrMapping('{"email":"email","name":"name"}');
    setSamlAutoRegister(true);
  };
  ...
  return (
    <>
        ...
        >
          ...
          >
            <h3 className="text-xs font-semibold mb-3">
              {editingOidc ? 'Edit OIDC Provider' : 'Add OIDC Provider'}
            </h3>
            <input
              value={oidcName}
            ...
            <input
              value={oidcSlug}
              onChange={(e) => setOidcSlug(e.target.value)}
              placeholder="Slug (e.g. keycloak)"
              className="w-full h-8 px-3 rounded-md border border-border bg-[#0a0a0a] text-xs mb-2 focus:outline-hidden focus:ring-1 focus:ring-primary/50"
            />
            <input
              value={oidcIssuer}
      ...
      {samlDialogOpen && (
        <div
          className="fixed inset-0 z-60 flex items-center justify-center bg-black/40"
          onClick={() => setSamlDialogOpen(false)}
        >
          <div
            className="w-full max-w-sm mx-4 p-5 rounded-xl border border-border bg-card shadow-2xl overflow-y-auto max-h-[80vh]"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-xs font-semibold mb-3">
              {editingSaml ? 'Edit SAML Provider' : 'Add SAML Provider'}
            </h3>
            <input
              value={samlName}
            ...
            <input
              value={samlSlug}
              onChange={(e) => setSamlSlug(e.target.value)}
              placeholder="Slug"
              className="w-full h-8 px-3 rounded-md border border-border bg-[#0a0a0a] text-xs mb-2 focus:outline-hidden focus:ring-1 focus:ring-primary/50"
            />
            <input
            ...
            <textarea
              ...
              placeholder="X.509 Certificate (PEM)"
              rows={3}
              className="w-full px-3 py-2 rounded-md border border-border bg-[#0a0a0a] text-xs mb-2 focus:outline-hidden focus:ring-1 focus:ring-primary/50 font-mono"
            />
            <input
              value={samlNameIdFmt}
              onChange={(e) => setSamlNameIdFmt(e.target.value)}
              placeholder="Name ID format"
              className="w-full h-8 px-3 rounded-md border border-border bg-[#0a0a0a] text-xs mb-2 focus:outline-hidden focus:ring-1 focus:ring-primary/50"
            />
            <input
              value={samlAttrMapping}
              onChange={(e) => setSamlAttrMapping(e.target.value)}
            ...
            <label className="flex items-center gap-2 mb-3 text-xs">
              <input
                type="checkbox"
                checked={samlAutoRegister}
                onChange={(e) => setSamlAutoRegister(e.target.checked)}
                className="rounded"
              />
              Auto-register users
            </label>
      ...
      <div>
            ...
            >
              ...
              <button
                onClick={() => {
                  ...
                  setOidcName(p.name);
                  setOidcSlug(p.slug);
                  setOidcIssuer(p.issuer_url);
                  setOidcClientId(p.client_id);
                  setOidcClientSecret('');
                  setOidcScopes(p.scopes);
                  setOidcDialogOpen(true);
                }}
        ...
        <div className="pt-4 mt-4 border-t border-border">
          ...
          <p className="text-[10px] text-muted-foreground/60 leading-relaxed">
            Configure any OpenID Connect provider (Keycloak, Okta, Auth0, Azure AD, etc.). The
            callback URL for all providers is:{' '}
            <code className="bg-muted px-1 rounded">
              {window.location.origin}/oauth/oidc/callback
            </code>
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-border">
            ...
            >
              ...
              <button
                onClick={() => {
                  ...
                  setSamlName(p.name);
                  setSamlSlug(p.slug);
                  setSamlEntityId(p.entity_id);
                  setSamlSsoUrl(p.sso_url);
                  setSamlCert('');
                  setSamlNameIdFmt(p.name_id_format);
                  setSamlAttrMapping(p.attribute_mapping);
                  setSamlAutoRegister(p.auto_register);
                  setSamlDialogOpen(true);
                }}
                className="p-1 rounded text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
              >
        ...
        <div className="pt-4 mt-4 border-t border-border">
          <h4 className="text-xs font-semibold mb-2 text-muted-foreground">How it works</h4>
          <p className="text-[10px] text-muted-foreground/60 leading-relaxed">
            Configure any SAML 2.0 identity provider (Keycloak, Okta, Azure AD, ADFS, etc.). The ACS
            URL is:{' '}
            <code className="bg-muted px-1 rounded">
