...
export async function callReducerLocal(reducer: string, args: unknown[]) {
  ...
  await fetch(
    `http://${import.meta.env.VITE_STDB_HOST || 'localhost:3001'}/v1/database/${DB_ID}/call/${reducer}`,
    {
      method: 'POST',
    ...
    if (child.nodeType === 3) {
      ...
      if (tag === 'strong' || tag === 'b') {
        ...
        content.push({
          type: 'text',
          text: c.textContent || '',
          marks: [{ type: 'link', attrs: { href: c.getAttribute('href') || '' } }],
        });
      } else if (tag === 'br') {
...
export function htmlToProseMirror(html: string): PMDoc {
  ...
  for (const node of div.childNodes) {
    ...
    if (
      ...
      doc.content.push({
        type: 'blockquote',
        content: [{ type: 'paragraph', content: extractInlineContent(el) }],
      });
    } else if (tag === 'pre') {
      ...
      if (img) {
        doc.content.push({
          type: 'image',
          attrs: { src: img.getAttribute('src') || '', alt: img.getAttribute('alt') || '' },
        });
      }
    } else if (tag === 'table') {
      const rows: PMNode[] = [];
      el.querySelectorAll(':scope > tr, :scope > thead > tr, :scope > tbody > tr').forEach((tr) => {
        ...
        trEl.querySelectorAll('th, td').forEach((td) => {
          ...
          cells.push({
            ...
            content: [{ type: 'paragraph', content: extractInlineContent(tdEl) }],
          });
        });
            ...
            if (cc.type === 'text') {
              ...
              if (cc.marks) {
                cc.marks.forEach((m) => {
                  ...
                  if (m.type === 'code') t = `\`${t}\``;
                  if (m.type === 'link')
                    t = `[${t}](${(m.attrs as { href?: string })?.href || ''})`;
                });
              }
              text += t;
            }
    ...
    if (marks) {
      for (const m of marks) {
        if (m.type === 'bold') t = `<strong>${t}</strong>`;
          ...
          t = `<a href="${(m.attrs as { href?: string })?.href || ''}">${t}</a>`;
      }
    }
    return t;
  }
