...
test.describe('Trash — restore lifecycle', () => {
  ...
  test('delete a page, restore it from trash, verify it returns', async ({ page }) => {
    ...
    // in trash and clicking one Restore leaves the others visible.
    const uniqueTitle = `Restore Me ${Date.now()}`;
    const pageUrl = await createPage(page, uniqueTitle, 'Restore content');
    if (!pageUrl) return;

    // ── 1. Move to trash via the sidebar context menu ──────────────────────
    await moveToTrash(page, uniqueTitle);
    ...
    await page.waitForLoadState('load');
    await expandUncategorized(page);
    await expect(page.locator('aside').getByText(uniqueTitle).first()).toBeVisible({
      timeout: 10000,
    });
  });

  test('permanently delete a page from trash', async ({ page }) => {
    const uniqueTitle = `Permanent Delete ${Date.now()}`;
    const pageUrl = await createPage(page, uniqueTitle, 'Permanent content');
    if (!pageUrl) return;
