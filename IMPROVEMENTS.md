...
## Status: All P0-P3 items complete, all P2 research items implemented. P4 SDK consolidation done. P4 access requests done. P4 image lightbox comments done. P4 shared page viewer route done. P4 per-share branding done. Entering maintenance/optimization phase.
...
| Date | Item | Commit |
|------|------|--------|
| 2026-06-27 | **P4 — Per-share branding!** Custom title override and logo URL per share link. STDB fields `brand_title` + `brand_logo_url` on ShareLink table, `update_share_branding` reducer. Share dialog has per-link branding editor. SharedPageView renders brand header and sets document title. | fbe7441e |
